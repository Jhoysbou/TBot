...
import com.jhoysbou.TBot.storages.MenuStorageImproved;
...
public class DefaultEditingService implements EditingService {
  private final MenuStorageImproved storage;
  private final TopicStorage subscriptionStorage;
  private final HashtagParser hashtagParser;

  @Autowired
  public DefaultEditingService(@Qualifier(value = "consistentMenuStorage") MenuStorageImproved storage,
  ...
  public void updateMenuItem(long id,
      ...
      Optional<Boolean> isSubscriptionRequred) throws ValidationException {
    ...
    responseText.ifPresent(response -> {
      ...
      hashtag.ifPresent(tag -> {
        // Changing topic names is not supported. News topics can only be created or
        // deleted
        // therefore we should create topic in NewsPreferenceStorage, otherwise throwing
        // exception.
    ...
    // }
    storage.updateMenuItem(id, trigger, responseText, isSubscriptionRequred);
  }
